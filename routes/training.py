...
from db import Question, UserQuizAttempt, db, Course, UserCourseProgress
...
@training_bp.route('/course/progress', methods=['POST'])
@login_required
def save_progress():
    ...
    if current_time > (progress.last_watched_timestamp + 15):
        return jsonify({'success': False, 'error': 'Avance de vídeo detectado.'}), 400
    progress.last_watched_timestamp = max(progress.last_watched_timestamp, current_time)
    course = Course.query.get(course_id)
    if progress.completed_at is None and progress.last_watched_timestamp >= (course.duration_seconds * 0.99):
...
def course_player_page(course_id):
    ...
    return render_template("training/course_player.html", course=course, quiz=course.quiz)
...
def submit_quiz(quiz_id):    
    ...
    quiz_questions = {q.id: q for q in Question.query.filter_by(quiz_id=quiz_id).all()}

    for key, user_submitted_value in request.form.items():
            ...
            if question.question_type == 'MULTIPLE_CHOICE':
                try:
                    ...
                    if correct_option and correct_option.id == user_answer_id:
                        correct_answers += 1
                        is_correct = True
                    user_answers[question_id] = {'answer': user_answer_id, 'is_correct': is_correct}
                except (ValueError, TypeError):
                    user_answers[question_id] = {'answer': user_submitted_value, 'is_correct': False}

            elif question.question_type == 'TEXT_INPUT':
                correct_answer_text = next((opt.text for opt in question.options), None)
                
                if correct_answer_text and user_submitted_value.strip().lower() == correct_answer_text.strip().lower():
                    correct_answers += 1
                    is_correct = True
                
                user_answers[question_id] = {'answer': user_submitted_value, 'is_correct': is_correct}
    ...
    flash(f"Avaliação concluída! Sua pontuação foi: {score:.2f}%", "success")
    return redirect(url_for('main.panel'))
...
def update_progress(course_id):
    ...
    progress.last_watched_timestamp = int(timestamp)
    db.session.commit()
    
    return jsonify(success=True)


@training_bp.route('/course/<int:course_id>/complete', methods=['POST'])
@login_required
def mark_course_as_completed(course_id):
    progress = UserCourseProgress.query.filter_by(user_id=current_user.id, course_id=course_id).first()
    ...
    return jsonify(success=True, message="Curso concluído com sucesso!")